      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-1 md:gap-2">
        {portfolioItems.map((item, index) => (
          <div
            ...
            className={`${item.column ? `lg:col-span-${item.column}` : ''} ${item.row ? `row-span-${item.row}` : ''} relative overflow-hidden rounded-sm border border-main/20 bg-background/70 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl`}
          >
            <div className="relative w-full">
              <Image
                ...
                width={2500}
                height={2500}
                className="object-cover object-center transition-transform duration-300 size-full scale-101 hover:scale-105"
              />
            </div>
            <div className="p-2 space-y-1">
              <div className={`group/view-detail flex items-center ${!item.link ? 'justify-start' : 'justify-between'} py-2`}>
                <p className="text-xs uppercase tracking-widest text-main/70 dark:text-sub/80">{item.tag}</p>
                ...
                {item.link && (
                  <a
                    ...
                    className="inline-flex items-center gap-2 group-hover/view-detail:flex-row-reverse text-sm text-main/70 dark:text-sub/80 transition-all ease-in-out duration-500 group-hover/view-detail:text-main dark:group-hover/view-detail:text-sub"
                  >
                    <span className="relative after:absolute after:left-0 after:-bottom-0 after:h-[1px] after:w-0 after:bg-current after:transition-all after:duration-500 group-hover/view-detail:after:w-full">
                      Xem chi tiết
                    </span>
                    <IconArrowRight className="w-4 h-4 transition-transform ease-in-out duration-500 group-hover/view-detail:translate-x-1" />
                  </a>
                )}
              </div>
